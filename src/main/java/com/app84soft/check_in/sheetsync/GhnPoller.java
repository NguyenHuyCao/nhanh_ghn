...
public class GhnPoller {
    ...
    public void run() {
        ...
        try {
            ...
            LocalDateTime from = (wm == null)
                    ? LocalDateTime.now().minusHours(cfg.getWindow().getGhnHours())
            ...
            while (true) {
                ...
                List<Map<String, Object>> orders = listOfMap(first(data.get("orders"),
                        data.get("items"),
                        data.get("data")));
                if (orders.isEmpty()) break;

                for (var m : orders) {
                    repo.upsertGhnOrder(m); // đã xử lý bỏ _PR trong impl
                }

                // WM: đặt về “ngay bây giờ” an toàn, vì GHN không trả trường updated chuẩn
                repo.saveGhnWatermark(LocalDateTime.now());

                Integer total = asInt(first(data.get("total"), data.get("total_orders")));
                Integer limit = asInt(first(data.get("limit"), 200));
                if (total == null || limit == null || page * limit >= total) break;
                page++;
            }
